...
const Hero = () => {
  ...
  return (
    <section id="home" className="min-h-screen flex items-center relative bg-gradient-hero overflow-hidden">
      ...
      <LightRays className="z-5" />
      
      {/* Scanner de energia */}
      <EnergyScanner className="z-6" scanInterval={20} />
      ...
      <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-20">
        <div className="text-center lg:text-left">
          ...
          <p className="hero-title inline-flex mb-5 px-4 py-2 text-xs uppercase tracking-[0.18em] text-accent border border-accent/40 bg-accent/10">
            {t('hero.legal')}
          </p>
          
          {/* Subtítulo com efeito hologram */}
          <HologramEffect delay={2} className="mb-6">
            <p className="hero-subtitle text-xl md:text-2xl text-text-gray font-light">
              {t('hero.subtitle')}
            </p>
          </HologramEffect>
          
          <p className="hero-description text-lg text-text-gray mb-10 max-w-lg">
            {t('hero.description')}
          </p>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce-custom z-20">
